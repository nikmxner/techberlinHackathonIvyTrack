...
import { XCircle, AlertTriangle, Euro, RotateCcw, CheckCircle } from 'lucide-react'
...
export function FailedItem({ transaction, isSelected, onSelect }: FailedItemProps) {
  ...
  const truncateErrorMessage = (message: string, maxLength: number = 60) => {
    ...
    >
      ...
      <div className="mb-2">
        <p className="text-sm text-red-800 leading-tight">
          {truncateErrorMessage(transaction.errorMessage || 'Unbekannter Fehler')}
        </p>
      </div>
